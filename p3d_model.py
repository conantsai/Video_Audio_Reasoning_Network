...
def downsample_basic_block(x, planes, stride):
    '''
    similar pooling - in order to reduce the number of parameters
    Args:
        x -> input image
        planes -> output channel
        stride -> move step
    Return:
        the image after downsampling
    '''
...
class P3D(nn.Module):
    ...
    def __init__(self, block, layers, modality='RGB',
        ...
        self.maxpool = nn.MaxPool3d(kernel_size=(2, 3, 3), stride=2, padding=0)
        # pooling layer for res2, 3, 4.
        self.maxpool_2 = nn.MaxPool3d(kernel_size=(
            2, 1, 1), padding=0, stride=(2, 1, 1))
        ...
        self.input_size = (self.input_channel, 16, 160,
                           160)       # input of the network
        self.input_mean = [0.485, 0.456, 0.406] if modality == 'RGB' else [0.5]
        self.input_std = [0.229, 0.224, 0.225] if modality == 'RGB' else [
            np.mean([0.229, 0.224, 0.225])]

    @property
    def scale_size(self):
        # asume that raw images are resized (340,256).
    ...
    def _make_layer(self, block, planes, blocks, shortcut_type, stride=1):
        ...
        layers.append(block(self.inplanes, planes, stride, downsample,
                            n_s=self.cnt, depth_3d=self.depth_3d, ST_struc=self.ST_struc))
        self.cnt += 1
