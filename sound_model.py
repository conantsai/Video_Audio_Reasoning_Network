...
class SoundNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x_obj = x.view(x.size(0), -1)(F.relu(self.conv8_1(x)))
        x_place = x.view(x.size(0), -1)(F.relu(self.conv8_2(x)))
        x = [x_obj, x_place]
        return x
...
for batch_idx, (input_tensor, target) in enumerate(train_loader):
    obj_target, place_target, = target[:1000], target[:, 1000:]
    input_tensor, obj_target, place_target = Variable(
        input_tensor), Variable(obj_target), Variable(place_target)

    optimizer.zero_grad()
    obj_out, place_out = net(input_tensor)
    loss_o = loss(obj_out, obj_target)
    loss_p = loss(place_out, place_target)
    loss_all = loss_o + loss_p
    loss.backward()
    optimizer.step()
